...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ jsPDF drawing helpers (exact replicas from sample) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ 3 ENGINEERING DRAWINGS FROM SAMPLE (Cross/Long/Plan) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function drawCrossSection(doc: any, inp: Inputs, r: Results) {
  ...
  doc.text('SECTION A-A â€” TRANSVERSE CROSS SECTION', W / 2, 15, { align: 'center' });
  ...
  doc.text(`${inp.numVents} VENTS @ ${f(inp.ventWidth)}m Ã— ${f(inp.ventHeight)}m`, mx(totW / 2), yRTL + 4, { align: 'center' });
...
function drawLongSection(doc: any, inp: Inputs, r: Results) {
  ...
  const yDT = my(deckTop), yRTL = my(inp.rtl), yHFL = my(inp.hfl), yGL = my(inp.gl), yFBL = my(r.fbl);
  doc.setFontSize(10); doc.setFont('helvetica', 'bold');
  doc.text('SECTION B-B â€” LONGITUDINAL SECTION', W / 2, 15, { align: 'center' });
  doc.setFontSize(6.5); doc.setFont('helvetica', 'normal');
...
function drawPlanView(doc: any, inp: Inputs, r: Results) {
  ...
  doc.setFontSize(6.5); doc.setFont('helvetica', 'normal');
  doc.text('Top-down view â€” showing vent arrangement and deck layout', W / 2, 20, { align: 'center' });
  doc.setFillColor(219, 234, 254);
  ...
  doc.setFontSize(5.5); doc.setFont('helvetica', 'normal');
  doc.text(`N_SPANS = ${inp.numSpans}  |  N_VENTS = ${inp.numVents}  |  VENT: ${f(inp.ventWidth)}m Ã— ${f(inp.ventHeight)}m`,
    ML + dW / 2, H - TB - MB - 4, { align: 'center' });
  titleBlock(doc, inp, 'PLAN VIEW', 'CS/DRG/03', '1:50 (SCHEMATIC)', '7', '169');
}

// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ CASE: 7-Sheet Mini Report (returns page count added = 7) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function addCase7Sheets(doc: any, autoTable: any, i: Inputs, r: Results, caseIndex: number, startPageNo: number, sampleTextLines: string[]) {
  ...
  doc.text(`${i.streamName}   â€”   ${i.location}`, W / 2, H / 2 + 12, { align: 'center' });
  doc.setFontSize(11); doc.setTextColor(245, 158, 11); doc.setFont('helvetica', 'bold');
  doc.text(`DESIGN DISCHARGE  Q = ${f(r.designDischarge)} mÂ³/s   (${r.governingMethod})`, W / 2, H / 2 + 34, { align: 'center' });
  const verdict = r.verdict.pass ? ['âœ“  PASS â€” COMPLIANT', [100, 220, 140]] : ['âš   REVIEW REQUIRED', [255, 160, 100]];
  ...
  doc.text(`Sheet ${sheet(1)} / 169   â€”   Submersible Causeway Design Report`, W / 2, H - 28, { align: 'center' });
  ...
  doc.text(`CASE ${String(caseIndex).padStart(2, '0')} â€” SHEET 2: STEP 1 DESIGN DISCHARGE (${i.set_id})`, W / 2, 17, { align: 'center' });
  ...
  titleBlock(doc, i, 'STEP 1 â€” DESIGN DISCHARGE', `CS/${i.set_id}/01`, 'â€”', sheet(2), '169');
  ...
  doc.setTextColor(0); doc.setDrawColor(0); border(doc);
  doc.setFontSize(13); doc.setFont('helvetica', 'bold');
  doc.text(`CASE ${String(caseIndex).padStart(2, '0')} â€” SHEET 3: HYDRAULIC DESIGN  (${i.set_id})`, W / 2, 17, { align: 'center' });
  ...
  titleBlock(doc, i, 'STEP 2 â€” HYDRAULIC', `CS/${i.set_id}/02`, 'â€”', sheet(3), '169');
  ...
  doc.setTextColor(0); doc.setDrawColor(0); border(doc);
  doc.setFontSize(13); doc.setFont('helvetica', 'bold');
  doc.text(`CASE ${String(caseIndex).padStart(2, '0')} â€” SHEET 4: STRUCTURAL DESIGN  (${i.set_id})`, W / 2, 17, { align: 'center' });
  ...
  titleBlock(doc, i, 'STEP 3 â€” STRUCTURAL', `CS/${i.set_id}/03`, 'â€”', sheet(4), '169');
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Simple polyline/chart helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ MAIN â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function main() {
  ...
  // Load all non-blank lines from sample .txt
  ...
  const sampleInput = {
    projectName: 'B.T to the R/f KB Road to P.Bheemavaram',
    streamName: 'Vented Submersible Causeway',
  ...
  for (let li = 0; li < sampleTextLines.length; li++) {
    ...
    // Page break before placing this line
    if (yPos > H - 50) {
